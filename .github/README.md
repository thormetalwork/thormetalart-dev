...
### Instructions (15) — `.github/instructions/`
...
| `blog.instructions.md`           | `data/wordpress/wp-content/themes/thormetalart/templates/single*.html`, `archive*.html`, `parts/sidebar*.html` | FSE blog templates, bilingual posts, sidebar      |
| `seo.instructions.md`            | `data/wordpress/wp-content/mu-plugins/tma-schema.php`, `tma-meta-tags.php`, `tma-sitemap.php`                  | Schema markup, meta tags, sitemap, GBP, local SEO |
| `workflows.instructions.md`      | `BACKLOG.md`, `.github/**`                                                                                     | Branching, PRs, quality gates                     |

### Agents (13) — `.github/agents/`

Domain-specific agents with restricted tool access.

| Agent                  | Tools                             | Purpose                                                      |
| ---------------------- | --------------------------------- | ------------------------------------------------------------ |
| `Blog Dev`             | read, edit, search, execute       | FSE blog templates, sidebar, bilingual posts (TICKET-WP-010) |
...
| `/promote-to-prod`   | DevOps               | Promote verified DEV changes to PROD         |
...
| -------------------- | ----------- | ------------------------------------- | ------------------------------------------------------------------------ |
| `safety-checks.json` | Config      | PreToolUse, SessionStart, PostToolUse | Destructive command guard, TDD reminder, security alerts, DEV-PROD guard |
